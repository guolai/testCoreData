...
+ (NSManagedObjectContext *)defaultContext {
    return [[CoreDataManager shareInstance] managedObjectContext];
}
@end
...
+ (NSArray *)whereInContext:(NSManagedObjectContext *)context format:(NSString *)format,...
{
...
+ (NSArray *)where:(id)condition startFrom:(int)ifrom limit:(int)limit sortby:(NSString *)strSort{
    ...
    return [self where:condition startFrom:ifrom limit:limit sortby:strSort
             inContext:[NSManagedObjectContext defaultContext]];
}

+ (NSArray *)where:(id)condition inContext:(NSManagedObjectContext *)context startFrom:(int)ifrom limit:(int)limit sortby:(NSString *)strSort
{
    return [self where:condition startFrom:ifrom limit:limit sortby:strSort
             inContext:context];
}

+ (NSArray *)where:(id)condition startFrom:(int)ifrom limit:(int)limit sortby:(NSString *)strSort inContext:(NSManagedObjectContext *)context {
    
    NSPredicate *predicate = ([condition isKindOfClass:[NSPredicate class]]) ? condition :
    [self predicateFromStringOrDict:condition];
    
    return [self fetchWithPredicate:predicate startFrom:ifrom limit:limit sortby:strSort 
                          inContext:context];
}
...
//add by bob 2013.5.1
+ (NSArray *)fetchWithPredicate:(NSPredicate *)predicate startFrom:(int)ifrom limit:(int)limit sortby:(NSString *)strSort inContext:(NSManagedObjectContext *)context {
    
    NSFetchRequest *request = [self createFetchRequestInContext:context];
    [request setPredicate:predicate];
    if(strSort)
    {
        NSArray *array = [strSort componentsSeparatedByString:@"^"];
        if(array.count > 0)
        {
            BOOL bAscend  = NO;
            if(array.count > 1)
                bAscend = [[array objectAtIndex:1] integerValue];
            NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:[array objectAtIndex:0] ascending:bAscend];
            [request setSortDescriptors:[NSArray arrayWithObjects:sortDescriptor,nil]];
        }
    }
    [request setFetchLimit:limit];
    [request setFetchOffset:ifrom];
    NSArray *fetchedObjects = [context executeFetchRequest:request error:nil];
    if (fetchedObjects.count > 0) return fetchedObjects;
    return nil;
}
...
- (BOOL)saveTheContext {
    ...
    if (!save || error) {
        BBINFO(@"Unresolved error in saving context for entityError: %@", error);
        return NO;
    }
